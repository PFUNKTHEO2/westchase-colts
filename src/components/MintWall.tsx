/**
 * The Mint Wall — live social proof, straight from Stripe via /api/mints.
 * Running count + dollars raised + the freshest mints as privacy-safe chips
 * (first name + last initial only; no photos of kids on the public wall).
 * Empty state sells the FIRST mint; a filled wall sells the NEXT one.
 */
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <h2 className="section-heading mb-2">
            The <span className="text-gradient">Mint Wall</span>
          </h2>
          {feed && feed.count > 0 ? (
            <p className="text-muted-foreground text-lg">
              <span className="font-bold text-foreground">{feed.count}</span>{" "}
              {feed.count === 1 ? "card" : "cards"} minted ·{" "}
            ...
            <p className="text-muted-foreground text-lg">
              Every card minted lands here, in front of the whole Colts community.
            </p>
          )}
        </motion.div>
        ...
        {feed && feed.count > 0 ? (
          <>
            {/* progress toward the season goal */}
              ...
              {feed.recent.map((m, i) => (
                <motion.div
                  ...
                  className={`rounded-lg border-2 px-4 py-3 bg-card min-w-[150px] text-left ${
                    m.variant === "metal" ? "border-accent/60" : "border-primary/60"
                  }`}
                >
                  <div className="flex items-center gap-1.5 mb-0.5">
                    <Sparkles className={`w-3.5 h-3.5 ${m.variant === "metal" ? "text-accent" : "text-primary"}`} />
                    <span className="text-[10px] font-bold tracking-wider uppercase text-muted-foreground">
                      {m.variant === "metal" ? "Metal mint" : "Digital mint"}
                    </span>
                  </div>
          ...
          <div className="text-center mb-8">
            <p className="text-foreground font-display font-bold text-xl mb-1">
              The first mint of the season is up for grabs.
            </p>
        ...
        <div className="text-center">
          <Button asChild size="lg" className="btn-gold bg-accent text-accent-foreground hover:bg-accent/90">
            <Link to="/create-card">Mint your Colt&apos;s card</Link>
          </Button>
        </div>
      </div>
    </section>
